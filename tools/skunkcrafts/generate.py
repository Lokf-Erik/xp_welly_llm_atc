#!/usr/bin/env python3
"""
SkunkCrafts Updater control-file generator for xp_wellys_atc.
...
backend_mode, api_key_saved / mistral_api_key_saved, the pilot callsign, ...).
Listing it in the oncelist ALONE preserves the "download only if absent"
behaviour without ever giving the updater a reason to overwrite a present copy.

This plugin is cloud-only in its shipped form, so the release tree carries no
Resources/models/. A user who built from source with local inference enabled
may still have ~2 GB of models under Resources/models/; those are deliberately
NOT tracked (anything not in the whitelist is left untouched by the updater),
so they survive every update. They must also stay out of the blacklist, which
DELETES.

Usage:
    python3 tools/skunkcrafts/generate.py \
        --tree  "<X-Plane>/Resources/plugins/xp_wellys_atc" \
        --version 4.2.1

Run it against the *release* tree you are about to publish (the same layout the
CI package job / `make install` produces), then commit/push that tree + the
control files to the `release` branch the cfg `module` URL points at.
"""
import argparse
import fnmatch
import os
...
IGNORE_GLOBS = [
    ...
    "Resources/espeak-ng-data/*",  # local-inference asset, not in the cloud bundle
    ".DS_Store",
    "**/.DS_Store",
...
def main() -> None:
    ...
    for dirpath, _dirs, files in os.walk(tree):
        for name in files:
            ...
            # flagged out-of-sync and overwritten. Kept present in the tree so
            # "download only if absent" still serves fresh installs.
