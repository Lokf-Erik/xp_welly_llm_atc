...
| **IFR readback content is not verified** — the plugin accepts any READBACK intent regardless of what altitude, flight level, fix name, or squawk code the pilot actually says. A pilot saying "descending FL090" when cleared to FL070 will not be corrected; a pilot checking in with "descending FL070, direct QA503" triggers the auto-clear only via frequency match, not content. Full readback enforcement (altitude, fix, SID/STAR identifier, squawk) is deferred to WhisperATC (fine-tuned aviation STT) because current models (Voxtral, base Whisper) garble identifiers too frequently to verify reliably without producing false rejections on correct readbacks. |
